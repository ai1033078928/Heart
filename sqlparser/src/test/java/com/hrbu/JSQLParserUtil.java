...
//            System.out.println(fromItem);
    ...
    @Test
    public void Main() throws JSQLParserException {
        /*String sql = "SELECT PAY_CERT_SUM_NO\n" +
                "  FROM (SELECT T.PAY_CERT_SUM_NO,\n" +
                "               T.FISCAL_YEAR,\n" +
                "               T.MOF_DIV_CODE,\n" +
                "               MAX(T.PAY_SUM_AMT) PAY_SUM_AMT,\n" +
                "               SUM(T2.XPAY_AMT) XPAY_AMT\n" +
                "          FROM PAY_CERT_NO_SUM_V1 T\n" +
                "          LEFT JOIN PAY_VOUCHER_BILL_V1 T2\n" +
                "            ON T.PAY_CERT_SUM_NO = T2.PAY_CERT_SUM_NO\n" +
                "           AND T.FISCAL_YEAR = T2.FISCAL_YEAR\n" +
                "           AND T.MOF_DIV_CODE = T2.MOF_DIV_CODE\n" +
                "         WHERE T.IS_DELETED = 2\n" +
                "           and t.fiscal_year = '2022'\n" +
                "           AND T2.IS_DELETED = 2\n" +
                "         GROUP BY T.PAY_CERT_SUM_NO, T.FISCAL_YEAR, T.MOF_DIV_CODE)\n" +
                " WHERE PAY_SUM_AMT != XPAY_AMT\n" +
                "   AND ROWNUM < 6;";*/

        String sql = "SELECT T.PAY_CERT_SUM_NO,\n" +
                "               T.FISCAL_YEAR,\n" +
                "               T.MOF_DIV_CODE,\n" +
                "               MAX(T.PAY_SUM_AMT) PAY_SUM_AMT,\n" +
                "               SUM(T2.XPAY_AMT) XPAY_AMT\n" +
                ...
                "           AND T2.IS_DELETED = 2\n" +
                "         GROUP BY T.PAY_CERT_SUM_NO, T.FISCAL_YEAR, T.MOF_DIV_CODE";
        List<String> strings = test_select_join(sql);
//        List<String> strings = test_select_table(sql);
